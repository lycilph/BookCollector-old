...
namespace BookCollector.Import
{
    ...
    public class ImportResultsViewModel : ReactiveScreen, IHandle<ImportMessage>
    {
        ...
        public void Ok()
        {
            var selected_books = Books.Where(b => b.IsSelected).Select(b => b.AssociatedObject.Book).ToList();
            book_repository.AddRange(selected_books);
        ...
        public void Handle(ImportMessage message)
        {
            ...
            Books = message.ImportedBooks.Select(b => new ImportedBookViewModel(b)).ToReactiveList();
            IsAllSelected = true;
        }
    }
}
