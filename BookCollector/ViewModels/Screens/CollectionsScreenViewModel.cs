...
{
    public class CollectionsScreenViewModel : ScreenBase, IHandle<ApplicationMessage>
    {
        ...
        public CollectionsScreenViewModel(IEventAggregator event_aggregator, ICollectionModel collection_model, ISnackbarMessageQueue message_queue, IDialogService dialog_service)
        {
        ...
        public override void Activate()
        {
            ...
            CollectionDescriptions = collection_model.GetAllCollectionDescriptions()
                                                     .Select(d => new DescriptionViewModel(d))
                                                     .ToReactiveList();

            // Get current collection
            if (collection_model.CurrentCollection != null)
                SelectedCollectionDescription = CollectionDescriptions.SingleOrDefault(d => d.Filename == collection_model.CurrentCollection.Description.Filename);
            else
        ...
        public void Handle(ApplicationMessage message)
        {
            if (message.Kind == ApplicationMessage.MessageKind.CollectionChanged)
                CanCancel = collection_model.CurrentCollection != null;
        }

        private void Continue()
        {
            collection_model.LoadCurrentCollection(SelectedCollectionDescription.Filename);
        ...
        {
            var collection = collection_model.CreateDefaultCollection();
            var description_view_model = new DescriptionViewModel(collection.Description);
            ...
            var vm = new DescriptionDialogViewModel(description_view_model, async (result) =>
            {
                ...
                if (result == MessageDialogResult.Affirmative)
                {
                    ...
                    collection_model.SaveCollection(collection);
                }
            });
            dialog.Content = vm;
        ...
        private async void EditCollectionAsync()
        {
            ...
            var vm = new DescriptionDialogViewModel(SelectedCollectionDescription, async (result) =>
            {
                ...
                // Update the actual collection, if needed
                if (result == MessageDialogResult.Affirmative)
                    collection_model.UpdateCollection(SelectedCollectionDescription.Obj);
            });
            dialog.Content = vm;
        ...
        private async void DeleteCollectionAsync()
        {
            ...
            collection_model.DeleteCollection(collection_to_remove.Obj);
        }
    }
}
