...
namespace BookCollector.Old
{
    public sealed class ImportAuthenticationStepViewModel : ReactiveScreen, IAuthenticationHandler
    {
        ...
        {
            var view = GetView() as ImportAuthenticationStepView;
            if (view == null)
                throw new Exception();

            return view.Browser.Document;
        }
        ...
        public void Navigating(NavigatingCancelEventArgs args)
        {
            ...
            authenticator.Navigating(args.Uri);
        }

        public void Navigated(NavigationEventArgs args)
        {
            if (authenticator == null)
                return;

            authenticator.Navigated(args.Uri);
        }

        public void Loaded(string html)
        {
            if (authenticator == null)
                return;

            authenticator.Loaded(html);
        }
    }
}
